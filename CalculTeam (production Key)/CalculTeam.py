...
import os
import re
from TransformToInput import transformFile
        ...
        with open(CONFIG_FILENAME) as fp:
            for line in fp:
                ...
                if param == "COMMENT":
                    ...
                    pattern = re.compile("[A-Z0-9a-z]*-[A-Z0-9a-z]*-[A-Z0-9a-z]*-[A-Z0-9a-z]*-[A-Z0-9a-z]*")
                    if pattern.match(value) == "None":
                        raise NameError("The API key isn't well formed!")
                    API_KEY = value
...
def main():
    setConfig()
    transformFile(INPUT_FILENAME, INPUTTMP_FILENAME)
    # Ouverture du fichier d'entree
            ...
            if str(response) != "404":
                for id, data in response.items():
                    for tmpData in data:
                        ...
                        actualPlayer.power = DIVISION_POINT[actualPlayer.division] + TIER_POINT[actualPlayer.tier] * 5
